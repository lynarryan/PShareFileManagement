...
int main(int argc, char **argv) {
    ...
    if (optind < argc) {
	    ...
	    manager = new FileManager(argv[optind+1]);
            network = new NetworkStructure(event_queue);
	     network->set_manager(manager);
        ...
        /* node connecting to a parent. min. 3 args: ip, port, key */
            ...
            if (argc - optind - 1 < 4) {
                ...
                std::cout <<
                    "+++ missing args: connect <ip|hostname> <port> <key> <directory>+++"
                << std::endl;
	    ...
	    char*  addiPath = argv[optind+4];
	    std::cout<< addiPath;

	    manager = new FileManager(&addiPath[0]);
